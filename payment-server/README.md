...
#### 1.6 Добавление столбцов в Google Sheets

Откройте таблицу и добавьте 4 новых столбца после существующих:

| Столбец | Название | Описание |
|---------|----------|----------|
| X | payment_status | Статус оплаты (Ожидает оплаты / Оплачено) |
| Y | payment_order_id | ID заказа в платежной системе |
| Z | payment_timestamp | Время оплаты (ISO формат) |
| AA | registration_timestamp | Время регистрации (timestamp) |
...
3. Проверьте столбец payment_status (X)
